...
## _render_impresora_doc — DESECHABLE, fase de PROPUESTA (2026-08-05).
##
## IMPRESORA-FOTOCOPIADORA DE OFICINA (de pie, ~1,20 m — GDD `impresora-documentos-tramite.md`,
## OBJ_008). NO hay fotocopiadora de pie en ningún pack ya descargado (`kaykit_furniture`,
## `kaykit_restaurant`: barridos por nombre, ninguna coincidencia con print/copier/copy — ver
## informe) ni tiempo/herramienta fiable para verificar y descargar un modelo nuevo de poly.pizza
## en esta pasada. VÍA DE COMPOSICIÓN (orden del coordinador, precedente canon
## `tools/_componer_props_soporte.gd`): la impresora de SOBREMESA YA EXISTENTE
## (`capturas/fuentes/props_poly/impresora.glb`, ya calibrada a 0,50 m real en
## `assets/sprites/mobiliario/impresora_<rot>.png`, SOLO LECTURA aquí) HORNEADA sobre la cajonera
## de oficina YA usada como soporte para cafetera/impresora/televisión (`Object_833` de
## `isometric_office.glb`, tapa a 0,675 m — mismo dato que `ID_RADIO`/`_render_cajonera_soporte.gd`).
## Alto total del conjunto ≈ 0,675 + 0,50 = 1,175 m ≈ el ~1,20 m pedido.
##
## Este script SOLO renderiza la cajonera SOLA (mismo criterio v2 que `_render_cajonera_soporte.gd`:
## calibración por altura real, pieza única, sin fusión 3D) — el aplanado con el PNG de la impresora
## de sobremesa ya existente lo hace `_componer_impresora_doc.gd` en 2D.
##
## Escribe SOLO al scratchpad de la sesión (NO `assets/`) — fase de PROPUESTA.

const SALIDA_SCRATCH := "C:/Users/manur/AppData/Local/Temp/claude/C--Users-manur-juego/3d17fabd-3e64-4077-8e28-18435fc15bba/scratchpad/"
...
## Misma pieza, mismo dato ya verificado que usa `ID_RADIO`/`_render_cajonera_soporte.gd`.
...
func _ready() -> void:
	...
	if escena == null:
		push_error("[IMPRESORA DOC] no se pudo cargar %s" % MODELO)
		get_tree().quit(1)
		return
	...
	for hijo: Node in raiz.find_children(NOMBRE_CAJONERA, "MeshInstance3D", true, false):
		...
		break
	if todas.is_empty():
		push_error("[IMPRESORA DOC] no se encontró %s" % NOMBRE_CAJONERA)
		get_tree().quit(1)
		return
	...
	var receta: Dictionary = {"id_salida": "cajonera_doc", "nombres": PackedStringArray([NOMBRE_CAJONERA])}
	...
	print("[IMPRESORA DOC] cajonera: radio=%.3f m, bruto 0°=%dpx alto -> objetivo=%.2fm (%.1fpx) -> factor=%.4f" % [
	...
	for i: int in ROTACIONES.size():
		var ruta: String = "%scajonera_doc_%d.png" % [SALIDA_SCRATCH, ROTACIONES[i]]
		(imagenes[i] as Image).save_png(ruta)
	print("[IMPRESORA DOC] guardado -> %scajonera_doc_{0,90,180,270}.png" % SALIDA_SCRATCH)
	print("[IMPRESORA DOC] hecho.")
	get_tree().quit()

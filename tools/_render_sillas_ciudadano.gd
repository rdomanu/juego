...
## _render_sillas_ciudadano — DESECHABLE (2026-08-07, ampliado 2026-08-07 2ª pasada). Renderiza
## las 2 sillas candidatas del ciudadano que pide `lado-de-accion.md` (madera=`chair_A_wood`,
## azul=`chair_C`), con 8 ROTACIONES cada una (cada 45°, no solo las 4 cardinales) — el bug de la
## silla mal orientada en la ventanilla viene de que el modelo es "de esquina": sus 4 caras reales
## caen en diagonal a cámara, así que un cardinal limpio (0/90/180/270) nunca deja el respaldo
## detrás del ciudadano sentado; hace falta poder elegir entre las 8 para encontrar el giro
## intermedio que sí lo hace (ver `design/art/lado-de-accion.md` §3, "regla del pipeline").
## Calibradas por ALTO real (~0,90 m con respaldo -> ~29 px, mismo criterio que
## `_render_biblioteca_fichajes.gd`/`render_props_poly.gd`, px = metros × 25,882 × 1,25 -- SIN
## inflar).
##
## Escribe SOLO al scratchpad -- PROHIBIDO tocar `assets/sprites/mobiliario/`.

const CARPETA_ORIGEN := "res://capturas/fuentes/kaykit_furniture/KayKit_Furniture_Bits_1.0_FREE/Assets/gltf/"
...
const ALTURA_OBJETIVO_M: float = 0.90

## Cada 45°, no cada 90° — ver la cabecera (piezas "de esquina").
const ROTACIONES_8: Array[int] = [0, 45, 90, 135, 180, 225, 270, 315]

const MODELOS: Array[Dictionary] = [
	{"id": "silla_espera_madera", "archivo": "chair_A_wood.gltf"},
	{"id": "silla_espera_azul", "archivo": "chair_C.gltf"},  # no existe variante _wood en el pack
]
...
func _cargar(contenedor: Node3D, modelo: Dictionary, todas: Dictionary) -> void:
	...
	if escena == null:
		push_error("[SILLAS CIUDADANO] no se pudo cargar %s" % ruta)
		return
	...
	for hijo: Node in raiz.find_children("*", "MeshInstance3D", true, false):
		...
		total += 1
	print("[SILLAS CIUDADANO] %s: %d MeshInstance3D cargadas" % [id, total])
...
func _ejecutar(todas: Dictionary) -> void:
	...
	print("[SILLAS CIUDADANO] px/metro efectivo = %.4f, objetivo alto = %.2f m -> %.2f px" % [
	...
	for modelo: Dictionary in MODELOS:
		...
		if nombres.is_empty():
			push_warning("[SILLAS CIUDADANO] %s: receta vacía -- se salta" % id)
			continue
		...
		print("[SILLAS CIUDADANO] %s: radio=%.3f m, bruto 0°=%dx%dpx (alto silueta=%dpx) -> factor=%.4f" % [
		...
		for i: int in ROTACIONES_8.size():
			...
			if err != OK:
				push_error("[SILLAS CIUDADANO] save_png '%s' fallo (error %d)" % [ruta, err])
			var img: Image = imagenes[i] as Image
			var alto_medido: int = _medir_alto(img)
			var dif: float = absf(float(alto_medido) - objetivo_px)
			print("[SILLAS CIUDADANO]   %s_%d.png: lienzo %dx%d px, ancla en (%.1f, %.1f), alto medido=%dpx (objetivo=%.1fpx, dif=%.1fpx) -> %s" % [
		...
		print("[SILLAS CIUDADANO] %s: guardado -> %s%s_{0,45,90,...,315}.png" % [id, CARPETA_SCRATCH, id])

	print("[SILLAS CIUDADANO] hecho.")
	get_tree().quit()

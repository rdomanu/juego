...
| 8 | **Espera de pie tras la llamada anticipada en ODAC** — usuario, 2026-07-30, jugando: *"en la odac está mucho tiempo esperando de pie desde que le llaman al ciudadano, esperando a que salga el anterior; ¿con cuánto tiempo de antelación se llama al siguiente?"*. **Dato verificado en código** (`src/core/flujo/flujo.gd`): el parámetro es `margen_llamada_anticipada_min` (Flujo #4, FL11) y vale **0.0** — nunca se cambia en ningún `.tres` ni config. La regla es "si `restante > camino + margen`: no llamar todavía", así que se llama al siguiente **exactamente** cuando lo que le queda de trámite al de delante iguala lo que el siguiente tarda en andar (margen extra CERO); en teoría llega justo. **Hipótesis**: el camino se estima desde el **centro** de la sala de espera (Flujo FL5, referencia fija), pero el ciudadano concreto puede estar sentado más cerca del puesto, así que llega **antes** de lo estimado; y como en ODAC la atención es **larga** (hasta 60 min una denuncia gorda), cualquier sobreestimación del camino se nota mucho más que en Documentación (trámites cortos). **Palancas propuestas** (sin decidir, sin cambiar ningún valor): (a) `margen_llamada_anticipada_min` **negativo** (Flujo #4) — llamar más tarde; (b) estimar el camino desde la **plaza real** de la persona en vez del centro de la sala. | balance/playtest | 1er playtest MVP | Abierta |
